...
def multi_dRNN_with_dilations(cells, inputs, dilations):
    ...
    outputs = []        #每个list维度为 'n_steps' tensors[batch_size,input_dim] 
    output = [] #每个list维度为 [batch_size,n_steps,input_dim]
    x = copy.copy(inputs)
    i = 0 
    for cell, dilation in zip(cells, dilations):
        scope_name = "multi_dRNN_dilation_%d" % i
        i +=1
        x= dRNN(cell, x, dilation, scope=scope_name)     # x是list的维度 ( n_steps, [batch_size, input_dims] )		
        outputs.append(x)
        x_trans = tf.stack(x,axis=0) #  x_trans的维度是n_steps*batch_size*input_dim
        x_trans = tf.transpose(x_trans, [1,0,2])   #x_trans的维度是batch_size*n_steps*input_dim
        output.append(x_trans)
    #last_outputs = tf.stack(last_outputs,axis=0)
	#last_outputs = tf.pack(last_outputs,axis=0)
