...
if __name__ == '__main__':
    sentence_file = 'trainfile2_word.txt'
    labels_file = 'trainfile2_labelsid.txt'
    length_file = 'trainfile2_length.txt'
    #sentence_file = 'testfile2_word.txt'
    #labels_file = 'testfile2_labelsid.txt'
    #length_file = 'testfile2_length.txt'
    max_length = 52
    word_list = w2v(sentence_file, max_length)
    label_list = process_to_ndarry(labels_file)
    length_list = process_to_ndarry(length_file)
    cPickle.dump([word_list, label_list, length_list], open('trainset2.p', 'w'))
    all_data = cPickle.load(open('trainset2.p', "r"))
    #cPickle.dump([word_list, label_list, length_list], open('testset2.p', 'w'))
    #all_data = cPickle.load(open('testset2.p', "r"))	
